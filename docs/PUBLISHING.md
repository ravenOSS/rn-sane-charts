...
This document is the **maintainer checklist** for shipping `@rn-sane-viz/core` and `@rn-sane-viz/rn` to the npm registry. External testers and app developers should **install from npm** (with a **dist-tag** such as `alpha`) rather than cloning the monorepo—see [GETTING_STARTED.md](GETTING_STARTED.md) and [README.md](../README.md).

## Why npm (not Git install) for feedback cohorts

- **Standard install path:** `pnpm add @rn-sane-viz/rn@alpha` (and `core` as a dependency).
...
1. **npm account** with permission to publish the **`@rn-sane-viz`** scope (create the org/scope on [npmjs.com](https://www.npmjs.com/) if needed).
...
|--------|----------------|--------------|
| `@rn-sane-viz/core` | `dist/` (CJS + ESM + `.d.ts` from **tsup**) | `exports["."].react-native` → `src/` for Metro |
| `@rn-sane-viz/rn` | `lib/` (react-native-builder-bob) | `source` / `src` as today |
...
pnpm typecheck
pnpm test
pnpm -r --filter @rn-sane-viz/core build
pnpm --filter @rn-sane-viz/rn run prepare   # bob → lib/

# Log in once: npm login
pnpm publish --filter @rn-sane-viz/core --access public --tag alpha --no-git-checks
pnpm publish --filter @rn-sane-viz/rn --access public --tag alpha --no-git-checks
```

- **`--tag alpha`:** consumers install with `@rn-sane-viz/rn@alpha` until you promote `latest`.
...
```bash
pnpm pack --filter @rn-sane-viz/core
pnpm pack --filter @rn-sane-viz/rn
```
